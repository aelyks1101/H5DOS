define(['doT'], function (doT) {
    ...
    var obj = {
        main: [
            ...
            '&#xe600;</div>',
            '<input type="text" data-com="explorer" data-cmd="location"/>',
            '<div' + ds + 'data-cmd="addFavorite" class="iconfont">&#xe602;</div>',
            '<div' + ds + 'data-cmd="close" class="iconfont close">&#xe60a;</div>',
            '</div>',
            ...
            '{{=it.language.filename[it.type]}}:',
            '<input type="text" data-com="explorer" data-cmd="filename"/>',
            '<select data-com="explorer" data-cmd="directorFilter"></select>',
            '<input type="button" data-com="explorer" data-cmd="{{=it.type}}"',
            ' value="{{=it.language[it.type]}}"/>',
            '<input type="button" data-com="explorer" data-cmd="close"',
            ' value="{{=it.language.cancel}}"/>',
            '</div>',
            '</div>'
        ],
        creatFolder: [
